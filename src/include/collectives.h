...
#define FUNC_INDEX(func, devredop, ncclType, al, pr)                           \
  (1 + ncclNumTypes +                                                          \
   (((((func) * ncclNumDevRedOps + (devredop)) * ncclNumTypes) + (ncclType)) * \
        NCCL_NUM_ALGORITHMS +                                                  \
    (al)) *                                                                    \
       NCCL_NUM_PROTOCOLS +                                                    \
   (pr))
...
#define NCCL_IMPL_NAME(func, algo, proto) nccl##func##algo##proto

/* Declare all collective operations */
#define DECL5(func, algo, proto, devredop, type)                              \
  extern __device__ void NCCL_FUNC_NAME(func, algo, proto, devredop, type)(); \
  extern __global__ void NCCL_KERN_NAME(func, algo, proto, devredop, type)(   \
