...
### 1. 技术定义
**一句话概括**：一种利用 RDMA WRITE 将接收端内存地址和凭证（rkey）直接推送到发送端预分配内存中的带内（In-Band）动态资源交互协议，旨在消除高频通信中的 TCP/控制面延迟。
...
### 1. 技术定义
**一句话概括**：摒弃内核中断，通过用户态纯自旋轮询（Polling）硬件完成队列（CQ）来感知数据收发状态，并依靠 events 计数器实现多 QP 屏障同步的非阻塞完成机制。
...
### 1. 技术定义
**一句话概括**：绕过重量级的 RDMA CM（通信管理库），直接使用轻量级 TCP Socket 作为带外通道（OOB）交换网卡 QPN、LID 等元数据，从而纯手动驱动 QP 完成 INIT -> RTR -> RTS 状态机跳转的建联设计。
...
### 1. 技术定义
**一句话概括**：在单一逻辑连接上绑定多个物理 QP 对，将单次发送的大数据块按 128 字节边界切片并分发到不同 QP 上，以此利用网络多路径（ECMP/LAG）来成倍提升单条连接的聚合带宽。
...
## 知识点五：Adaptive Routing (AR) 与 WR 分离

### 1. 技术定义
**一句话概括**：在自适应路由场景下，为了防止带有完成标记（IMM）的报文因乱序早于数据报文到达，强行将“数据写入（RDMA WRITE）”与“完成通知（0字节 WRITE_WITH_IMM）”拆分为两条按序提交的 WR，利用 QP 的顺序发送特性保障语义正确。
...
## 知识点六：MR 注册三条路径与池化复用 (MR Cache)

### 1. 技术定义
**一句话概括**：一个基于物理页对齐和引用计数（refs）的设备级缓存池，拦截高频的硬件内存注册/注销调用，确保同一块显存在极度并发下安全复用，消除控制面系统调用瓶颈。
...
## 知识点七：GDR Flush（PCIe 写入可见性保证）

### 1. 技术定义
**一句话概括**：利用 PCIe Non-Posted Read（读操作）必须与前置 Posted Write（写操作）严格保序的硬件规范，通过一个本地 Loopback QP 发起一字节的无效读取，强迫 PCIe 交换机写缓存刷入 GPU 显存，填补物理可见性鸿沟。
...
### 3. 深度释疑：为什么是“接收端”发起？什么是 Loopback QP？为什么要读那块地？
**疑问**：数据是发送端发过来的，那是谁负责去发起这个 Flush？什么是“自己连自己的 Loopback QP”？读出来的内容有用吗？
**物理逻辑解谜**：
